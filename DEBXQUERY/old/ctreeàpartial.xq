...
declare function local:For($for,$groupby,$orderby,$where,$return,$context,$static)
{
  ...
  then 
      ...
      (:<partial type="return">{$return/values/value/partial/node()}</partial>:)</partial>}</value>  
  else 
      <value><content></content><partial type="For">{$var}{$path}{(:$context:)}<position>{$i}</position><partial type="where">{$rwhere/values/value/partial/node()}</partial>
...
declare function local:exp($exp,$context,$static)
{
   ...
   let $partial :=
   (if (count($exp/*)>1) then  for-each($exp/*,function($x){local:exp($x,$context,$static)/values/value/partial/node()})
   else $exp)
   return
...
<values>{<value><partial type="CElem">
     {$exps/values/value/partial/node()}</partial>
...
{
$x/content,
<partial type="StaticFuncCall">{$gflwor/values/value/partial/node()}</partial> 
}
</value>
}</values>
</StaticFuncCall>
