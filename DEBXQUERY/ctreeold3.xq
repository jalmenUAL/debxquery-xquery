declare function local:For($for,$items,$context)
{
  ...
  <values>{
  for $x in 
  local:exp($path,$context) return <value>{$x//value/*}</value>
  }</values>
  }</IterPath></for>
  )
  return
...
declare function local:Let($let,$items,$context)
{
  ...
  <values>{
  <value>{local:exp($path,$context)//value/*}</value>
  }</values>
  }</IterPath></let>
  )
  return
...
declare function local:For2($for,$items,$context)
{  
...
declare function local:Let2($let,$items,$context)
{  
  if ($let/IterPath)
  then
  let $ncontext := 
...
<bib>
 {
  let $b := db:open('bstore1')/bib/book
  for $a in $b
