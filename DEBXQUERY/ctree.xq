...
{  
  if ($quan/@type="some") then let $res := local:GFLWOR($quan/*,$context,$static)
                               ...
                               <Quantifier>                          
                   <values><value>{local:GFLWOR($quan/*,$context,$static)/values/value/path}<content>{some $r in $res/values/value/content/text() satisfies $r="true"}</content></value></values></Quantifier>
  else let $res := local:GFLWOR($quan/*,$context,$static) 
                   return 
                   <Quantifier>
                   <values><value>{local:GFLWOR($quan/*,$context,$static)/values/value/path}<content>{every $r in $res/values/value/content/text() satisfies $r="true"}</content></value></values></Quantifier>
};
...
{
  if (empty($where/*)) then 
  <Where><values><value><path>{$context,$where/*,<item>{true()}</item>}</path><content>{true()}</content></value></values></Where>  
  else 
...
{  
   ...
   if (local:exp($step,$context,$static)/values/value/content/text()="true") then
   "true()"
   else
   if (local:exp($step,$context,$static)/values/value/content/text()="false") then
   "false()"
   else
   let $values := local:exp($step,$context,$static)/values/value/content
   return
   if ($values) then
      serialize(<root>{$values}</root>, map {'method': 'xml' }) || "/content/node()" 
   else "()"
}; 
...
{  
   ...
   if (name($step)="FnNot") 
    ...
    if (name($step)="Quantifier") 
          then $step/@type || " " || $step/GFLWOR/For/Var/@name || " in " || local:showPath($step/GFLWOR/For/*[2],$context)
          || " satisfies " || local:showPath(($step/GFLWOR)/*[2],
             <context>{
   (for $x in $context/* where not($x//@name=$step/GFLWOR/For/Var//@name) return $x) union <var><name>{$step/GFLWOR/For/Var}</name><path>{$step/GFLWOR/For/*[2]}</path><context>{$context}</context><position>{0}</position></var>}</context>)
...
<CElem><values>{<value>{local:exps(tail($query/*),$context,$static)/values/
 value/path}<content>{
 element {data($query/QNm/@value)} {local:exps(tail($query/*),$context,$static)/values/
 value/content/(node()|@*)}}</content></value>
}</values></CElem>  
};

declare function local:CAttr($query,$context,$static)
{ 
<CAttr><values>{<value>{local:exps(tail($query/*),$context,$static)/values/
 value/path}<content>{
...
if (local:exp($cond,$context,$static)/values/value/text()=true()) then 
    <If><values><value><path><If>{local:exp($cond,$context,$static)/values/value/path}</If><Then>{local:exp($then,$context,$static)/values/value/path}</Then></path>{local:exp($then,$context,$static)/values/value/content}</value></values></If> 
    else <If><values><value><path><If>{local:exp($cond,$context,$static)/values/value/path}</If><Else>{local:exp($else,$context,$static)/values/value/path}</Else></path>{local:exp($else,$context,$static)/values/value/content}</value></values></If> 
};
...
declare function local:epaths($string_query,$type)
{
  ...
  if (not(local:showPath($path,$context)="()")) then
   "Empty path found in '" || local:showPath($path,$context) || "' where " || 
  local:print_context($context)
...
xquery:parse("<bib>
 {
  for $a in db:open('bstore1')/bib/book
  let $b := $a
  where some $a in $b/publisher satisfies $a='Addison-Wesley'
  return
    <book year='{ $b/@year }'>
     { $b/title }
    </book>
